...
describe('FrameItem', () => {
	describe('getItems', () => {
		const setup = () => {
			...
			return {
				frameItem: new FrameItem(
					...
					id,
					{}
				), getItemsWithinFrameSpy, id, boardId, childId
			}
		}
		...
		it.each(['app_card', 'card', 'document', 'embed', 'frame', 'image', 'shape', 'sticky_note', 'text'] as const)('should call the api and pass the type param (%s)',
			(type) => {
				const {frameItem, getItemsWithinFrameSpy, id, boardId} = setup()
