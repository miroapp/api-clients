...
describe('Entrypoint test', () => {
    ...
    describe('constructor', () => {
        ...
        it('can receive options from the parameters', () => {
            const miro = new Miro({clientId: '123', clientSecret: '123', redirectUrl: '123'})
    ...
    describe('getAuthUrl', () => {
        it('returns the correct url', () => {
            expect(testMiro().getAuthUrl()).toEqual('https://miro.com/oauth/authorize?response_type=code&client_id=id&redirect_uri=url&team_id=&state=');
        })

        it('returns the correct url with the state', () => {
            expect(testMiro().getAuthUrl('someState')).toEqual('https://miro.com/oauth/authorize?response_type=code&client_id=id&redirect_uri=url&team_id=&state=someState');
        })

        it('returns the correct url for the team', () => {
            expect(testMiro().getAuthUrl(undefined, 'someTeamId')).toEqual('https://miro.com/oauth/authorize?response_type=code&client_id=id&redirect_uri=url&team_id=someTeamId&state=');
        })
    })
    ...
    describe('isAuthorized', () => {
        it('returns the correct values', async () => {
            const userId = 'foo'
            const miro = testMiroWithStored(userId, {userId: 'bar', accessToken: 'token'})
