...
ITEMDATAPLATFORMCREATEITEMSINBULKUSINGFILEFROMDEVICE_ONE_OF_SCHEMAS = [
    "AppCardDataResponse",
    "CardData",
...
class ItemDataPlatformcreateitemsinbulkusingfilefromdevice(BaseModel):
    """
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = ItemDataPlatformcreateitemsinbulkusingfilefromdevice.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in ItemDataPlatformcreateitemsinbulkusingfilefromdevice with oneOf schemas: AppCardDataResponse, CardData, DocumentDataResponse, EmbedDataResponse, ImageDataResponse, ShapeData, StickyNoteData, TextData. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in ItemDataPlatformcreateitemsinbulkusingfilefromdevice with oneOf schemas: AppCardDataResponse, CardData, DocumentDataResponse, EmbedDataResponse, ImageDataResponse, ShapeData, StickyNoteData, TextData. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Union[
        ...
        if not matches:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into ItemDataPlatformcreateitemsinbulkusingfilefromdevice with oneOf schemas: AppCardDataResponse, CardData, DocumentDataResponse, EmbedDataResponse, ImageDataResponse, ShapeData, StickyNoteData, TextData. Details: "
                + ", ".join(error_messages)
            )
