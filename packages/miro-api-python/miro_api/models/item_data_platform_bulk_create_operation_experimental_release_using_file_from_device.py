...
ITEMDATAPLATFORMBULKCREATEOPERATIONEXPERIMENTALRELEASEUSINGFILEFROMDEVICE_ONE_OF_SCHEMAS = [
    "AppCardDataResponse",
    "CardData",
...
class ItemDataPlatformBulkCreateOperationExperimentalReleaseUsingFileFromDevice(
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in ItemDataPlatformBulkCreateOperationExperimentalReleaseUsingFileFromDevice with oneOf schemas: AppCardDataResponse, CardData, DocumentDataResponse, EmbedDataResponse, ImageDataResponse, ShapeData, StickyNoteData, TextData. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in ItemDataPlatformBulkCreateOperationExperimentalReleaseUsingFileFromDevice with oneOf schemas: AppCardDataResponse, CardData, DocumentDataResponse, EmbedDataResponse, ImageDataResponse, ShapeData, StickyNoteData, TextData. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Union[
        ...
        if not matches:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into ItemDataPlatformBulkCreateOperationExperimentalReleaseUsingFileFromDevice with oneOf schemas: AppCardDataResponse, CardData, DocumentDataResponse, EmbedDataResponse, ImageDataResponse, ShapeData, StickyNoteData, TextData. Details: "
                + ", ".join(error_messages)
            )
